...
function createBlockElement(block) {
    ...
    const linkIndicator = block.linked ? `<span class="link-icon">${ICON_LINK}</span>` : '';
...
function saveBlockChanges() {
    ...
    const card = document.querySelector(`.block-card[data-id="${currentEditingId}"]`);
    const nameEl = card.querySelector(".block-name");
    const linkHtml = block.linked ? `<span class="link-icon">${ICON_LINK}</span>` : '';
    nameEl.innerHTML = linkHtml + block.name;
    
    card.querySelector(".block-time").textContent = block.time;
    closeEditPanel();
}
...
function deleteBlock(id) {
    ...
    // Trigger the CSS removal animation
