...
async function loadFamousCases() {
    try {
        const res = await fetch('cases.yaml');
        const text = await res.text();
        FAMOUS_CASES = parseYamlCases(text);
    } catch {}
    setPlaceholderCase();
}

function parseYamlCases(yaml) {
    const cases = [];
    let current = null;
    for (const line of yaml.split('\n')) {
        ...
        if (trimmed.startsWith('- ')) {
            ...
            if (val.startsWith('p:')) current.p = val.slice(2).trim();
            if (val.startsWith('d:')) current.d = val.slice(2).trim();
        } else if (current && trimmed.startsWith('p:')) {
            current.p = trimmed.slice(2).trim();
        } else if (current && trimmed.startsWith('d:')) {
            current.d = trimmed.slice(2).trim();
        }
    }
    if (current) cases.push(current);
    return cases;
